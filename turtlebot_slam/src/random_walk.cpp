...
public:
	...
	RandomWalk(ros::NodeHandle& nh) :
		...
		frontier_publisher = nh.advertise < sensor_msgs::PointCloud
				> ("frontiers", 1);
...
//					ROS_INFO("Second queue starting");
					...
					if (isFrontier(q[0], q[1])) {
						...
						std::vector<int> neighbors = getNeighbors(q[0], q[1]);
...
//			ROS_INFO("end of isFrontier(p)");
			std::vector<int> neighbors = getNeighbors(p[0], p[1]);
			...
			for (int i = 0; i < neighbors.size(); i += 2) {
				...
				!vec_contain(map_close_list, v)) {
					...
					std::vector<int> neighborsV = getNeighbors(v[0], v[1]);
