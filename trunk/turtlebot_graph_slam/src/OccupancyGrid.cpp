...
int reformat(int i)
{
    return (int)(i/RESOLUTION + SIZE/2);
}
...
//			break;
        ...
        // Put the max range measurements in the occupancy grid
            sensor_msgs::LaserScan::ConstPtr scan = laserscansaver[i];
            int num_ranges = scan->ranges.size();
            float incr = scan->angle_increment;
            float minAngle = scan->angle_min;
            float maxRange = scan->range_max;
            double angle;
            for (int j = 0; j <= num_ranges; j++){
                if(scan->ranges[j] == maxRange) {
                    angle = (double)(j*incr)+minAngle;
                    double x = scan->ranges[j] * cos(angle);
                    double y = scan->ranges[j] * sin(angle);

                    int grid_x = (int)((x+xPose)/RESOLUTION + SIZE/2);
                    int grid_y = (int)((y+yPose)/RESOLUTION + SIZE/2);
                    ...
                    og.data[((grid_y*og.info.width)+grid_x)] = 0;
