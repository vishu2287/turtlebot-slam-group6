...
//tf::TransformListener listener;
  const static double MIN_SCAN_ANGLE_RAD = -100.0/180*M_PI;
  const static double MAX_SCAN_ANGLE_RAD = +100.0/180*M_PI;
  const static float PROXIMITY_RANGE_M = 1; // Should be smaller than sensor_msgs::LaserScan::range_max
double feature_extractor (const sensor_msgs::LaserScan::ConstPtr& msg){
